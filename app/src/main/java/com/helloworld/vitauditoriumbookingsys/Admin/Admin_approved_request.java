...
public class Admin_approved_request extends AppCompatActivity {

    ArrayList<AllEventsHelperClass> allTicketsList;
    RecyclerView recyclerView;
    Admins_Approved_Request_Adapter adtv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_admin_approved_request);

        allTicketsList = new ArrayList<>();
        recyclerView = findViewById(R.id.approved_request_recyclerView);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        adtv = new Admins_Approved_Request_Adapter(Admin_approved_request.this, allTicketsList);
                ...
                for (DataSnapshot fds: snapshot.getChildren()) {
                    if (fds.getKey().startsWith("Room")) {
                        for (DataSnapshot sds: fds.getChildren()) {
                            ...
                            assert abthc != null;
                            if (abthc.getStatus().equals("Approved")) {
